use crate::entities::{Entry, Bullet, BulletType};
use anyhow::Result;
...
mod tests {
    ...
    fn test_round_trip_consistency() {
        ...
        for bullet_type in [
            ...
            assert_eq!(original_bullets.len(), parsed_bullets.len(), 
                      "Bullet count mismatch for {:?}", bullet_type);
                      
            for (orig, parsed) in original_bullets.iter().zip(parsed_bullets.iter()) {
                assert_eq!(orig.content, parsed.content, 
                          "Content mismatch for {:?}", bullet_type);
                assert_eq!(orig.bullet_type, parsed.bullet_type,
                          "Type mismatch for {:?}", bullet_type);
            }
        }
    }
    ...
    fn test_whitespace_handling() {
        ...
        let content_with_whitespace = "  # Tasks  \n  Complete task  \n\n  # Events  \n  Meeting today  \n\n\n";
